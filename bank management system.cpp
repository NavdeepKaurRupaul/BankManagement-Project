#include<iostream>
#include<fstream>
#include<cctype> //it conatins toupper()
#include<iomanip> //it contains setw()
using namespace std;
...
class account{
    ...
    public:
    	void create_account(){
        	...
        	cin>>type;
        	type=toupper(type); //to convert type in upper cases
        ...
        void report() const{
        	...
        	//setw function to show data in tabular format
        }	
        int retacno() const{
        	return acno;
        }
...
void modify_account(int n){
	...
	while(!File.eof() && found==false){
		...
		if(ac.retacno()==n){
			...
			File.seekp(pos,ios::cur);//moves put pointer
...
void delete_account(int n){
	...
	inFile.seekg(0,ios::beg);//moves get pointer
	while(inFile.read((char *) (&ac), sizeof(account))){
		if(ac.retacno()!=n){
...
//intriductionn function
void intro(){
	cout<<"\n\n\n\t  BANK";
	cout<<"\n\n\tMANAGEMENT";
	cout<<"\n\n\t  SYSTEM";
	cout<<"\n\n\n\nMADE BY : NAVDEEP"<<endl;
	cin.get();
}
